...
import { EncounterState } from '@prisma/client';
import { prisma } from '../../infrastructure/db/prisma.js';
import { ensureGuildCampaign, exportCampaign, importCampaign } from '../../application/campaignBundle.js';
import { resolvePrototypeEnemyAction, resolvePrototypePlayerBatch, runPrototypeStressSimulation } from '../../domain/combat/prototype.js';
...
export const rpgCommand = {
    data: new SlashCommandBuilder()
        ...
        .setRequired(true)
        .addChoices({ name: 'Fighter', value: 'FIGHTER' }, { name: 'Support', value: 'SUPPORT' })))
        .addSubcommand((subcommand) => subcommand
        .setName('view')
...
async function exportCampaignCommand(interaction) {
...
async function importCampaignCommand(interaction) {
...
export const battleCommand = {
    data: new SlashCommandBuilder()
        ...
        .setRequired(true)
        .addChoices({ name: prototypeActions.STRIKE, value: 'STRIKE' }, { name: prototypeActions.HEAL, value: 'HEAL' }, { name: prototypeActions.NOTHING, value: 'NOTHING' }))
    ...
    async execute(interaction) {
        ...
        if (subcommand === 'create')
            await createBattle(interaction);
        else if (subcommand === 'join')
            await joinBattle(interaction);
        else if (subcommand === 'start')
            await startBattle(interaction);
        else if (subcommand === 'action')
            await submitAction(interaction);
        else if (subcommand === 'enemy-action')
            await submitEnemyAction(interaction);
        else if (subcommand === 'stress-test')
            await runStressTest(interaction);
        else
            await showBattleStatus(interaction);
    }
};
...
async function createBattle(interaction) {
    if (!isAdministrator(interaction)) {
...
async function joinBattle(interaction) {
...
async function startBattle(interaction) {
    if (!isAdministrator(interaction)) {
...
async function resolveIfEveryoneSubmitted(interaction, encounterId) {
    ...
    if (submissions.length < encounter.participants.length)
        return;
    const playerState = {
        enemyHp: encounter.enemyHp,
        enemyMaxHp: encounter.enemyMaxHp,
        players: encounter.participants.map((participant) => ({ id: participant.characterId, name: participant.character.name, class: participant.character.class, currentHp: participant.currentHp, maxHp: participant.character.maxHp, dead: participant.currentHp <= 0 }))
    };
    const actions = submissions.map((submission) => {
    ...
    await prisma.$transaction(async (transaction) => {
        ...
        await transaction.resolutionEvent.create({ data: { encounterId: encounter.id, round: encounter.round, phase: EncounterState.RESOLVE_PLAYERS, sequence: 1, eventType: 'PROTOTYPE_PLAYER_RESOLUTION', payload: result.events, definitionVersions: {} } });
    });
...
async function submitEnemyAction(interaction) {
    if (!isAdministrator(interaction)) {
    ...
    await prisma.$transaction(async (transaction) => {
        ...
        await transaction.resolutionEvent.create({ data: { encounterId: encounter.id, round: encounter.round, phase: EncounterState.ENEMY_ACTION, sequence: 2, eventType: 'PROTOTYPE_ENEMY_RESOLUTION', payload: result.events, definitionVersions: {} } });
    });
    await interaction.reply(result.events.map((event) => event.message).join('\n'));
}
async function runStressTest(interaction) {
    if (!isAdministrator(interaction)) {
...
async function showBattleStatus(interaction) {
    const encounter = await getActiveEncounter(interaction);
    if (encounter === null) {
...
async function getActiveEncounter(interaction) {
...
function requireGuildId(interaction) {
    if (interaction.guildId === null) {
