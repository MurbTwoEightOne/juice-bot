...
export async function ensureGuildCampaign(guildId) {
...
export async function exportCampaign(guildId) {
...
export async function importCampaign(guildId, bundle) {
    ...
    await prisma.$transaction(async (transaction) => {
        ...
        for (const move of bundle.moves) {
            const moveData = { ...move, class: move.class, effects: toInputJson(move.effects) };
        ...
        for (const character of bundle.characters) {
            ...
            const savedCharacter = existingCharacter === null
                ? await transaction.playerCharacter.create({ data: { guildId, userId: character.userId, name: character.name, level: character.level, class: character.class, utilityKitId: character.utilityKitId, maxHp: character.maxHp, currentHp: character.currentHp } })
                : await transaction.playerCharacter.update({ where: { id: existingCharacter.id }, data: { guildId, name: character.name, level: character.level, class: character.class, utilityKitId: character.utilityKitId, maxHp: character.maxHp, currentHp: character.currentHp, activeEncounterId: null } });
...
//# sourceMappingURL=campaignBundle.js.map